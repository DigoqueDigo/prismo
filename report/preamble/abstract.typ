...
By supporting various data manipulation techniques, the evaluation of these systems becomes increasingly complex, given that workloads need to meet a series of criteria that validate deduplication and compression simultaneously, while ensuring that characteristics common to storage systems remain within the scope of the evaluation, in particular the spatial and temporal locality of accesses.


However, the benchmarks available to the community (#link("https://fio.readthedocs.io/en/latest/")[fio], #link("https://www.oracle.com/downloads/server-storage/vdbench-downloads.html")[vdbench]) only allow partial manipulation of entropy and deduplication levels. Furthermore, trace simulation is overly simplistic and becomes impractical in modern systems because they are too fast to complete the trace and there is no trivial way to extend it while preserving its characteristics.


Furthermore, in order to extract maximum performance, some systems only provide low-level I/O interfaces, such as #link("https://spdk.io/")[SPDK], which makes it even more complicated to execute workloads, as the aforementioned benchmarks do not directly support such protocols for disk communication.


That said, this dissertation aims to develop a benchmark for storage systems that is capable of supporting various I/O interfaces, as well as generating realistic workloads that allow the collection of relevant metrics for system evaluation, thus enabling the identification of performance bottlenecks and impacts associated with the characteristics of the system itself.
